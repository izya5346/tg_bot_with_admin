...
class Coupon(Model):
    ...
    async def pay_to_refs(self):
        try:
            ...
            user1.balance += Decimal(amount['value']
                                     ['coupon']) * Decimal('0.1')
            hs = History(type='ref', amount=Decimal(
                amount['value']['coupon']) * Decimal('0.1'), user=user1)
            await hs.save()
            await user1.save()
        except Exception as e:
            print(e)
        try:
            amount = await Setting.get(label='Стоимость').values()
            user2 = await User.get(refcode=user1.refered_by)
            user2.balance += Decimal(amount['value']
                                     ['coupon']) * Decimal('0.05')
            hs = History(type='ref', amount=Decimal(
                amount['value']['coupon']) * Decimal('0.05'), user=user2)
            await hs.save()
            await user2.save()
        except Exception as e:
            print(e)
